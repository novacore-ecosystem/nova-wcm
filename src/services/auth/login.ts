 ...
 * `useTenantLoginConfiguration`). Also requires X-App-Key (`env.appCode`'s doc comment) — unlike
 * nova-console's Root console, nova-wcm has no Root-bypass path, so this header is always sent,
 * never conditionally omitted; a blank/misconfigured `env.appCode` surfaces as the backend's own
 * "missing app code" error rather than silently resolving to Root behavior.
 */
